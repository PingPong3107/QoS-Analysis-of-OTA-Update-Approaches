...
class SimPollingIntervalsDefaultTestNonDelta extends Simulation {
  ...
  setUp(
    adminDeletesRepos
      ...
      .andThen(registerScen.inject(atOnceUsers(Metadata.numberOfCars)))
      .andThen(createGroupAndRollout.inject(atOnceUsers(1)))
      .andThen(pollingScen.inject(atOnceUsers(Metadata.numberOfCars)))
  )
